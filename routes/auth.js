...
/* Constant-time string compare so we don't leak timing info on the fallback path. */
...
/* Emergency override: if ADMIN_USER + ADMIN_PASS are set in the server's
   environment, that combo can always log in as owner — even if
   data/accounts.json gets wiped, corrupted, or reset by the host.
   Leave these unset in normal operation; only set them if you get locked out. */
function checkEnvOverride(user, pass){
...
router.post('/login', (req, res) => {
  ...
  if(match && bcrypt.compareSync(pass, match.passHash)){
    ...
    return res.json({ ok: true, user: match.user, role: match.role });
  }

  if(checkEnvOverride(user, pass)){
    console.log('[auth] Logged in via ADMIN_USER/ADMIN_PASS environment override.');
    req.session.user = user;
    req.session.role = 'owner';
    return res.json({ ok: true, user, role: 'owner' });
  }
